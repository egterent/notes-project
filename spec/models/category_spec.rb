  ...
  context 'methods' do  
    describe 'nested_notes_count' do
      context 'if there are not any nested notes' do
        subject { create(:category_with_subcategories, user: user) }
      ...
      context 'if there are nested notes in subcategories' do
        let(:subs_count) { 2 }
        let(:notes_count) { 3 }
        subject { create(:category_with_subcategories_and_notes, subs_count: subs_count, notes_count: notes_count) }
        ...
        context 'when a note is deleted' do
          it 'should decrement nested notes count' do
            expect { subject.subcategories.first.notes.first.destroy }.to change { subject.nested_notes_count }.by(-1)
          end
        end

        context 'when a new note is added' do
          it 'should increment nested notes count' do           
            expect { subject.subcategories.first.notes.create!(title: 'note', user_id: user.id) }
    ...
    describe 'each' do
      ...
      context 'if  category contains subcategories' do
        subject { create(:category_with_two_levels_subcategories, subs_count: 2, subs_2_count: 2) }

        it 'should return the category with all subcategories' do
          expect(subject.each).to contain_exactly(subject, subject.subcategories.first,
